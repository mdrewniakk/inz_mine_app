...
@st.cache_data
def get_data():
    ROI = ee.Geometry.Point(-63.392211, 7.455763)
    start_year = 2018
    end_year = 2022
    years = ee.List.sequence(start_year, end_year)
    images_ven = years.map(lambda year: best_image(ee.Number(year), ROI))
    images_ven = images_ven.add(latest_image(ROI))
    dataset = calc_indices(images_ven, get_bound())
    return dataset
...
def get_bound():
    ...
    return mining.filter(ee.Filter.eq('system:index', '0000000000000000354a'))


st.header("Cerro Bolivar - Wenezuela")

row1_col1, row1_col2, row1_col3 = st.columns([1, 2, 1])

Map = geemap.Map(center=(7.455763, -63.392211), zoom=13)
