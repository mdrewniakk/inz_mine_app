...
def get_data():
    ...
    images_rpa = years.map(lambda year: best_image(ee.Number(year), ROI))
    images_rpa = images_rpa.add(latest_image(ROI))
    dataset = calc_indices(images_rpa, get_bound())
    return dataset
...
def get_bound():
    ...
    return mining.filter(ee.Filter.eq("system:index", "00000000000000002655"))
